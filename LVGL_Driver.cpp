...
static lv_disp_t* display = nullptr;  // 新增：保存显示对象引用
...
void Lvgl_Init(void)
{
  ...
  disp_drv.rotated = LV_DISP_ROT_NONE;  // 初始化为无旋转，等待配置加载后设置
  
  display = lv_disp_drv_register( &disp_drv );  // 保存显示对象引用
...
void Lvgl_SetRotation(int rotation) {
    if (display == nullptr) {
        printf("[LVGL] Display not initialized, cannot set rotation\n");
        return;
    }
    
    lv_disp_rot_t lvgl_rotation;
    
    // 将角度转换为LVGL旋转常量
    switch (rotation) {
        case 0:
            lvgl_rotation = LV_DISP_ROT_NONE;
            break;
        case 90:
            lvgl_rotation = LV_DISP_ROT_90;
            break;
        case 180:
            lvgl_rotation = LV_DISP_ROT_180;
            break;
        case 270:
            lvgl_rotation = LV_DISP_ROT_270;
            break;
        default:
            printf("[LVGL] Invalid rotation angle: %d\n", rotation);
            return;
    }
    
    printf("[LVGL] Setting display rotation to %d degrees\n", rotation);
    
    // 设置显示旋转
    lv_disp_set_rotation(display, lvgl_rotation);
    
    // 强制刷新整个屏幕以应用新的旋转
    lv_obj_invalidate(lv_scr_act());
    
    printf("[LVGL] Display rotation applied successfully\n");
}
